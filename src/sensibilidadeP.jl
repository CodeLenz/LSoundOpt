# ===================================================================================
# Calcula a derivada P-Norm Absoluta
#
function DerivadaP(ne,nn,γ::Vector{T0},connect::Matrix{T1},coord::Matrix{T0},
                  ...
                  elements_design::Vector,A::Vector,p0=20E-6; p_norm=8.0) where {T0,T1,T2}

    d = zeros(ne)
    Nf = length(freqs)
    nt = length(nodes_target)

    # --- PRÉ-CÁLCULO PARA O PESO ESPECTRAL ---
    ...
    for i = 1:Nf
        ...
        # Razão Absoluta: P_atual / p0^2
        Rn_values[i] = A[i] * (P2_avg / p0^2)
    end
    ...
    for f in freqs
        ...
        # --- PESO ESPECTRAL (Sem referência externa) ---
        # Quanto maior a pressão P2avg, maior será este fator.
        # Frequências com SPL baixo terão peso quase zero.
