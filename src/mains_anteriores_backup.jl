...
"""
function Otim_ISLP_antes_separar(arquivo::String, freqs::Vector, vA::Vector; verifica_derivada=false)
   ...
   if occursin(".geo",arquivo)
    ...
    if !verifica_derivada && raio_filtro > 0
         ...
         vizinhos, pesos, n_min_viz, n_max_viz = Vizinhanca(ne,centroides,raio_filtro,elements_design)
    end 
    ...
    for iter = 1:niter
        ...
        # Se raio>0, filtra a sensibilidade do objetivo 
        if raio_filtro > 0 
            dΦ_filt = Filtro(vizinhos,pesos,dΦ,elements_design)
            Lgmsh_export_element_scalar(arquivo_pos,dΦ_filt,"dΦ_filt")  
            # Sensibilidade 
    ...
    println(fd,"Raio do filtro ",raio_filtro)
    println(fd,"Número nínimo de vizinhos ",n_min_viz)
    println(fd,"Número máximo de vizinhos ",n_max_viz)
    close(fd)
    

    # Retorna os históricos, para visualização imediata
